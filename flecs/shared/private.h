...
#include "flecs_lua.h" // <- STEVE CHANGE
...
// STEVE CHANGE
int lua_absindex (lua_State * L, int arg);
int lua_rawgeti_t (lua_State * L, int arg, int i);
int lua_rawgetp (lua_State * L, int arg, const void * p);
int lua_getfield_t (lua_State * L, int arg, const char * k);
int lua_getuservalue (lua_State * L, int arg);

int lua_geti (lua_State * L, int arg, lua_Integer i);

int lua_isinteger (lua_State * L, int arg);

void lua_rawsetp (lua_State * L, int arg, const void * p);
void lua_setuservalue (lua_State * L, int arg);

void luaL_setfuncs (lua_State * L, const luaL_Reg * funcs, int n);

int luaL_len (lua_State * L, int arg);

int luaL_getsubtable (lua_State * L, int arg, const char * k);
...
static inline bool getentity (lua_State *L, int arg, ecs_entity_t *e)
{
    #if UINTPTR_MAX == UINT32_MAX
        void *data = LUA_TUSERDATA == lua_type(L, arg) ? luaL_checkudata(L, arg, "flecs_entity_t") : NULL;
    #else
    ...
    if (data && e) memcpy(e, data, sizeof(*e));

    return data != NULL;
}

static inline void push_entity (lua_State *L, ecs_entity_t e)
{
    #if UINTPTR_MAX == UINT32_MAX
        ecs_entity_t *data = lua_newuserdata(L, sizeof(e));

        *data = e;

        luaL_newmetatable(L, "flecs_entity_t");
...
static inline ecs_entity_t checkentity(lua_State *L, int arg)
{
    ecs_entity_t e;

    luaL_argcheck(L, getentity(L, arg, &e), arg, "invalid entity");
...
bool ecs_lua_iter_next(lua_State *L, int idx); // STEVE CHANGE
