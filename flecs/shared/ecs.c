...
{
    if (arg < 0) arg = lua_gettop(L) + arg + 1;
...
{
  if (lua_getfield_t(L, arg, k) == LUA_TTABLE)
    ...
    lua_setfield(L, arg, k);  /* assign new table to field */
...
{
    bool had_len = luaL_callmeta(L, arg, "__len");
...
int luaopen_plugin_ecs(lua_State *L) // <- STEVE CHANGE
{
    // luaL_checkversion(L); <- STEVE CHANGE
    /*luaL_newlibtable(L, ecs_lib)*/lua_newtable(L); // <- STEVE CHANGE
