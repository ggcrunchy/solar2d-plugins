...
int new_entity(lua_State *L)
{
    ...
    else if(args == 1) /* entity | name(string) */
    {
        ...
        if(/*lua_isinteger(L, 1)*/getentity(L, 1, &e)) { /* empty */ } // e = luaL_checkinteger(L, 1); <- STEVE CHANGE
        else if(type == LUA_TSTRING) name = luaL_checkstring(L, 1);
        else return luaL_argerror(L, 1, "expected entity or name");
    }
    else if(args == 2)
    {
        if(/*lua_isinteger(L, 1)*/getentity(L, 1, &e)) /* entity, name (string) */ // STEVE CHANGE
        {
            luaL_checktype(L, 2, LUA_TSTRING);

            // e = luaL_checkinteger(L, 1); <- STEVE CHANGE
...
int delete_entity(lua_State *L)
{
    ...
    if(/*lua_isinteger(L, 1)*/getentity(L, 1, &entity)) // STEVE CHANGE
    {
        // entity = luaL_checkinteger(L, 1); <- STEVE CHANGE
        ecs_delete(w, entity);
    }
