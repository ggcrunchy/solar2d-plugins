...
luaL_reg win_image_methods[] = {
	{
		"assign", [](lua_State * L)
		{
			fipWinImage & wi = WinImage(L, 1);

			if (IsType(L, "fipWinImage", 2)) wi = WinImage(L, 2);
		...
		"captureWindow", [](lua_State * L)
		{
			lua_pushboolean(L, WinImage(L, 1).captureWindow((HWND)lua_touserdata(L, 2), (HWND)lua_touserdata(L, 3))); // wi, hwnd1, hwnd2, ok
		...
		"copyToClipboard", [](lua_State * L)
		{
			HWND win = (HWND)lua_touserdata(L, 2);

			lua_pushboolean(L, WinImage(L, 1).copyToClipboard(win));// wi[, win], ok
		...
		"getToneMappingOperator", [](lua_State * L)
		{
			FREE_IMAGE_TMO tmo;
			double p1, p2, p3, p4;

			WinImage(L, 1).getToneMappingOperator(&tmo, &p1, &p2, &p3, &p4);

			lua_pushinteger(L, tmo);	// wi, tmo

			GetName(L, FI_EnumList::kTMO, -1);

			for (auto n : { p1, p2, p3, p4 }) lua_pushnumber(L, n);	// wi, tmo, p1, p2, p3, p4
		...
		"isValid", [](lua_State * L)
		{
			lua_pushboolean(L, WinImage(L, 1).isValid());	// wi, valid

			return 1;
		}
	}, {
		"pasteFromClipboard", [](lua_State * L)
		{
			lua_pushboolean(L, WinImage(L, 1).pasteFromClipboard());// wi, ok

			return 1;
		}
	}, {
		"setToneMappingOperator", [](lua_State * L)
		{
			FREE_IMAGE_TMO tmo = (FREE_IMAGE_TMO)GetCode(L, FI_EnumList::kTMO, 2);

			double p[4] = { 0.0, 0.0, 1.0, 0.0 };

			for (int i = 0; i < 4; ++i)
			{
				if (lua_isnumber(L, 3 + i)) p[i] = lua_tonumber(L, 3 + i);
			}

			WinImage(L, 1).setToneMappingOperator(tmo, p[0], p[1], p[2], p[3]);
...
luaL_Reg win_image_constructors[] = {
	{
		"NewWinImage", [](lua_State * L)
		{
			const char * name = luaL_optstring(L, 1, "BITMAP");

			lua_pushstring(L, name);// ..., name
			lua_insert(L, 1);	// name, ...

			FREE_IMAGE_TYPE type = (FREE_IMAGE_TYPE)GetCode(L, FI_EnumList::kType, 1);

			unsigned w = luaL_optinteger(L, 2, 0U), h = luaL_optinteger(L, 3, 0U), bpp = luaL_optinteger(L, 4, 0U);

			fipWinImage * wi = New<fipWinImage>(L, "fipWinImage", GetImageMethods(), win_image_methods); // wi

			new (wi) fipWinImage(type, w, h, bpp);
...
int FI_LoadWinImage (lua_State * L)
{
	luaL_register(L, NULL, win_image_constructors);
