...
template<typename T> T & Get (lua_State * L, int index = -1)
{
	return *(T *)lua_touserdata(L, index);
}

template<typename T> T & Check (lua_State * L, const char * name, int index = -1)
{
	return *(T *)luaL_checkudata(L, index, name);
}

template<typename T> T & Check (lua_State * L, const char * name, const char * alt, int index = -1)
{
	lua_getmetatable(L, index);	// ..., object_mt
	luaL_getmetatable(L, alt);	// ..., object_mt, alt_mt

	int eq = lua_equal(L, -2, -1);

	lua_pop(L, 2);	// ...

	if (eq) return *(T *)lua_touserdata(L, index);
...
template<typename T> T * New (lua_State * L, const char * name, luaL_Reg methods[], luaL_Reg * more = NULL)
{
	void * ud = lua_newuserdata(L, sizeof(T)); // ..., ud

	if (luaL_newmetatable(L, name)) // ud, meta
	{
		luaL_register(L, NULL, methods);

		if (more) luaL_register(L, NULL, more);
...
#define FIP_DECLARE_GETTER(name) fip##name & name (lua_State * L, int index);

FIP_DECLARE_GETTER(Image)
FIP_DECLARE_GETTER(MemoryIO)
FIP_DECLARE_GETTER(MetadataFind)
FIP_DECLARE_GETTER(MultiPage)
FIP_DECLARE_GETTER(Tag)

#ifdef _WIN32
	FIP_DECLARE_GETTER(WinImage)
#endif
