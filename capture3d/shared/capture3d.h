...
struct Model {
    ...
    struct RenderInfo {
        ...
        Vec4f (RenderInfo::*mVertex)(const Scene &, RenderState &, const Model & model, const Matrix & mat, int, int);
        ...
        Vec4f basic_vertex (const Scene & scene, RenderState & rs, const Model & model, const Matrix & mat, int iface, int nthvert);
        Vec4f diffuse_vertex (const Scene & scene, RenderState & rs, const Model & model, const Matrix & mat, int iface, int nthvert);
        Vec4f diffuse_normals_vertex (const Scene & scene, RenderState & rs, const Model & model, const Matrix & mat, int iface, int nthvert);
...
struct Group {
    ...
    bool IsDestroyed (void) const { return mSubGroups.size() == 1U && mSubGroups.front() == this; }

    void MarkDestroyed (void)
    {
        mSubGroups.clear();

        mSubGroups.push_back(this);
    }
...
struct Scene {
	...
	Vec3f mUp{0, 1, 0};
	int mComps, mW, mH;
	int mBlobRef{LUA_NOREF}, mGroupRef;
...
Group & GetGroup (lua_State * L, int arg = 1);
Model & GetModel (lua_State * L, int arg = 1);
Object & GetObject (lua_State * L, int arg = 1);
Scene & GetScene (lua_State * L, int arg = 1);
Texture<unsigned char> & GetTexture (lua_State * L, int arg = 1);
Texture<float> & GetFloatTexture (lua_State * L, int arg = 1);

void AddConstructor (lua_State * L, const char * name, lua_CFunction func, int nupvalues = 0);
