...
static void par_init_tables()
{
    ...
    par_msquares_binary_point_table = PAR_CALLOC_1(int*, 16); // <- STEVE CHANGE
    par_msquares_binary_triangle_table = PAR_CALLOC_1(int*, 16); // <- STEVE CHANGE
    for (int i = 0; i < 16; i++) {
        int ntris = *binary_token - '0';
        binary_token++;
        par_msquares_binary_triangle_table[i] =
            PAR_CALLOC_1(int, (ntris + 1) * 3); // <- STEVE CHANGE
        int* sqrtris = par_msquares_binary_triangle_table[i];
        sqrtris[0] = ntris;
        int mask = 0;
        int* sqrpts = par_msquares_binary_point_table[i] = PAR_CALLOC_1(int, 7); // <- STEVE CHANGE
        sqrpts[0] = 0;
    ...
    int* quaternary_values = PAR_CALLOC_1(int, strlen(QUATERNARY_TABLE)); // <- STEVE CHANGE
    ...
    quaternary_values = PAR_CALLOC_1(int, strlen(QUATERNARY_EDGES)); // <- STEVE CHANGE
...
{
    par_gray_context* context = (par_gray_context*) contextptr;
    int i = (int)PAR_CLAMP(context->width * x, 0, context->width - 1); // <- STEVE CHANGE
    int j = (int)PAR_CLAMP(context->height * y, 0, context->height - 1); // <- STEVE CHANGE
...
static float color_height(float x, float y, void* contextptr)
{
    ...
    return context->data[k * 4 + 3] / 255.0f; // <- STEVE CHANGE
}
...
void par_msquares_free(par_msquares_meshlist* mlist)
{
    ...
    for (int i = 0; i < mlist->nmeshes; i++) {
        PAR_FREE(meshes[i]->points);
        PAR_FREE(meshes[i]->triangles);
        PAR_FREE(meshes[i]);
    }
    PAR_FREE(meshes);
    PAR_FREE(mlist);
...
// STEVE CHANGE
static pthread_mutex_t mutex_tables = PTHREAD_MUTEX_INITIALIZER;
// /STEVE CHANGE
    ...
    par_msquares_height_fn heightfn)
{
	...
	pthread_mutex_lock(&mutex_tables);	// <- STEVE CHANGE
    if (!par_msquares_binary_point_table) {
        par_init_tables();
    }
	pthread_mutex_unlock(&mutex_tables);// <- STEVE CHANGE
    ...
    for (int row = 0; row < nrows; row++) {
        ...
        vertsx[1] = vertsx[5] = 0.5f * normalized_cellsize; // <- STEVE CHANGE
        ...
        vertsy[3] = vertsy[7] = normalized_cellsize * (row + 0.5f); // <- STEVE CHANGE
    ...
    PAR_FREE(edgemap);
    PAR_FREE(prevrowmasks);
    PAR_FREE(prevrowinds);
    ...
    if (flags & PAR_MSQUARES_SIMPLIFY) {
        ...
        PAR_FREE(tris);
        tris = newtris;
        PAR_FREE(simplification_codes);
        PAR_FREE(simplification_tris);
        PAR_FREE(simplification_ntris);
    ...
    int height, int cellsize, int bpp, int flags)
{
    ...
    for (int row = 0; row < nrows; row++) {
        vertsx[0] = vertsx[6] = vertsx[7] = 0;
        vertsx[1] = vertsx[5] = vertsx[8] = 0.5f * normalized_cellsize; // <- STEVE CHANGE
        vertsx[2] = vertsx[3] = vertsx[4] = normalized_cellsize;
        vertsy[0] = vertsy[1] = vertsy[2] = normalized_cellsize * (row + 1);
        vertsy[4] = vertsy[5] = vertsy[6] = normalized_cellsize * row;
        vertsy[3] = vertsy[7] = vertsy[8] = normalized_cellsize * (row + 0.5f); // <- STEVE CHANGE
        int northi = row * cellsize * width;
        int southi = PAR_MIN(northi + cellsize * width, maxrow);
        ...
        for (int col = 0; col < ncols; col++) {
            ...
            for (int c = 0; c < 4; c++) {
                ...
                counts |= ntris[c] << (8 * c);
                par_msquares__mesh* mesh = mlist->meshes[color];
                float height = (mesh->color >> 24) / 255.0f; // <- STEVE CHANGE
    ...
    PAR_FREE(simplification_blocks);
    PAR_FREE(simplification_ntris);
    PAR_FREE(simplification_tris);
    PAR_FREE(simplification_words);
...
{
    PAR_FREE(polygon->points);
    PAR_FREE(polygon->chains);
    PAR_FREE(polygon->lengths);
    PAR_FREE(polygon);
}
    ...
    par_msquares_mesh const* mesh)
{
    ...
    PAR_FREE(hemesh.verts);
    PAR_FREE(hemesh.edges);
    PAR_FREE(hemesh.sorted_edges);
    return result;
}
// STEVE CHANGE
#undef ASSERT
