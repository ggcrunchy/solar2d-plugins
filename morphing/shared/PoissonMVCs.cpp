...
*/
	...
	if(fabs(c.cx-p.x) < 1E-10 && fabs(c.cy-p.y) < 1E-10) {
		for(int i = 0;  i < int(poly.size());  i++) {
...
void basicPoissonMVCs(const vector<Point2D> &poly, const vector<int> & edge, const Point2D &p, vector<double> &coords, BaseCircle c) {
	if(checkPolygon(poly) == false) {
		return;
	}
//	BaseCircle c;
	minCircle(poly, c);
	poissonMVCs(poly, edge, p, coords, c);
}