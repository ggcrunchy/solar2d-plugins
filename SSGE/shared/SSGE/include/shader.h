...
struct PBRShader : public IShader {
    ...
    Vector3f fragment(float u, float v) override{
        ...
        std::vector<Vector3f> F/*[*/(maxLights)/*]*/; // <- STEVE CHANGE
        std::vector<float>  NDF/*[*/(maxLights)/*]*/; // <- STEVE CHANGE
        std::vector<float>  G/*[*/(maxLights)/*]*/; // <- STEVE CHANGE
        
        //Storing in array for vectorizing
        std::vector<Vector3f> radianceLights/*[*/(maxLights)/*]*/; // <- STEVE CHANGE
        std::vector<Vector3f> interpLightDir/*[*/(maxLights)/*]*/; // <- STEVE CHANGE
        std::vector<Vector3f> halfwayDir/*[*/(maxLights)/*]*/; // <- STEVE CHANGE
        std::vector<float>  nDotL/*[*/(maxLights)/*]*/; // <- STEVE CHANGE
        std::vector<Vector3f> numerator/*[*/(maxLights)/*]*/; // <- STEVE CHANGE
        std::vector<float>  invDenominator/*[*/(maxLights)/*]*/; // <- STEVE CHANGE
        std::vector<Vector3f> specular/*[*/(maxLights)/*]*/; // <- STEVE CHANGE
        std::vector<Vector3f> kD/*[*/(maxLights)/*]*/; // <- STEVE CHANGE
