...
{
 size_t len;
 const char *s=bytes_checklstring/*luaL_checklstring*/(L,1,&len); // <- STEVE
...
#define PACKNUMBER(OP,T)			\
   ...
   {						\
    ...
    /* STEVE CHANGE: luaL*/bob_addlstring(&b,(void*)&a,sizeof(a));	\
    break;					\
   }
...
#define PACKSTRING(OP,T)			\
   ...
   {						\
    ...
    /*STEVE CHANGE: luaL*/bob_addlstring(&b,(void*)&ll,sizeof(ll));	\
    /*STEVE CHANGE: luaL*/bob_addlstring(&b,a,l);			\
    break;					\
   }
...
// /STEVE CHANGE
 while (*f)
 {
  ...
  while (N--) switch (c)
  {
   ...
   case OP_ZSTRING:
   {
    size_t l;
    const char *a=luaL_checklstring(L,i++,&l);
    /*luaL*/bob_addlstring(&b,a,l+(c==OP_ZSTRING)); // <- STEVE CHANGE
    break;
   }
 ...
 /*luaL*/bob_pushresult(&b); // <- STEVE CHANGE
 return 1;
}
...
#else
 luaL_openlib(L, NULL/*LUA_STRLIBNAME*/, R, 0);	// STEVE CHANGE
#endif
 return 0;
}
