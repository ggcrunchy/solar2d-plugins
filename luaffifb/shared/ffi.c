...
#define TO_NUMBER(TYPE, ALLOW_POINTERS)                                     \
    ...
    case LUA_TUSERDATA:                                                     \
        ...
        if (ct.type == INVALID_TYPE) {                                      \
            ...
            ret = (TYPE)check_intptr(L, idx, p, &ct);                       \
        }                                                                   \
        lua_pop(L, 1);                                                      \
        break;                                                              \
                                                                            \
...
{ TO_NUMBER(double, 0); return (float)ret; }
...
complex_double check_complex_double(lua_State* L, int idx)
{
    ...
    case LUA_TUSERDATA:
        ...
        if (ct.type == COMPLEX_DOUBLE_TYPE) {
            ...
            real = (double)check_intptr(L, idx, p, &ct);
        }
        lua_pop(L, 1);
        break;
...
#define POW(l,r,s) s = (int64_t)pow((double)l, (double)r)
...
static int ffi_errno(lua_State* L)
{
    ...
    if (!lua_isnoneornil(L, 1)) {
        ...
        jit->last_errno = (int32_t)luaL_checknumber(L, 1);
    } else {
        lua_pushinteger(L, jit->last_errno);
    }
...
static int ffi_number(lua_State* L)
{
    ...
    if (ct.type != INVALID_TYPE) {
        lua_pushinteger(L, (lua_Integer)check_intptr(L, 1, data, &ct));
        return 1;
    } else {
...
int luaopen_plugin_ffi(lua_State* L)
{
    lua_settop(L, 0);
