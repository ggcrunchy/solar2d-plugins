...
jo_gif_t jo_gif_start(/*const char *filename*/JO_File * gf, short width, short height, short repeat, int numColors) {	// <- STEVE CHANGE
...
void jo_gif_frame(jo_gif_t *gif, unsigned char * rgba, short delayCsec, bool localPalette) {
	...
	// STEVE CHANGE
	std::vector<unsigned char> ip(size);

	unsigned char *indexedPixels = ip.data();//(unsigned char *)malloc(size);
	// /STEVE CHANGE
	{
		// STEVE CHANGE
		std::vector<unsigned char> dp(size * 4);
		unsigned char *ditheredPixels = dp.data();//(unsigned char*)malloc(size*4);
		// /STEVE CHANGE
		...
		// free(ditheredPixels);	// <- STEVE CHANGE
	}
	if(gif->frame == 0) {
		// Global Color Table
	...
	// /STEVE CHANGE
	++gif->frame;
	// free(indexedPixels);	// <- STEVE CHANGE
}
