...
// STEVE CHANGE
//extern void jo_write_mpeg(FILE *fp, const void *rgbx, int width, int height, int fps);
// /STEVE CHANGE
...
// STEVE CHANGE
	...
	else if(fps <= 30) fp->PutC(0x15);
	else if(fps <= 50) fp->PutC(0x16);
	else fp->PutC(0x18); // 60fps
	fp->Write("\xFF\xFF\xE0\xA0", 4);

	fp->Write("\x00\x00\x01\xB8\x80\x08\x00\x40", 8); // GOP header
	fp->Write("\x00\x00\x01\x00\x00\x0C\x00\x00", 8); // PIC header
	fp->Write("\x00\x00\x01\x01", 4); // Slice header
// /STEVE CHANGE
