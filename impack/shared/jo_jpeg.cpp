...
// STEVE CHANGE
#include "jo_file.h"
#include "jo_gif.h" // header file abuse: has jo_write_mpeg declaration
// /STEVE CHANGE
...
static void jo_writeBits(/*FILE*/JO_File *fp, int &bitBuf, int &bitCnt, const unsigned short *bs) { // <- STEVE CHANGE
	bitCnt += bs[1];
	bitBuf |= bs[0] << (24 - bitCnt);
	while(bitCnt >= 8) {
		unsigned char c = (bitBuf >> 16) & 255;
		/*putc*/fp->PutC(c);//, fp); // <- STEVE CHANGE
		if(c == 255) {
			/*putc*/fp->PutC(0);//, fp); // <- STEVE CHANGE
		}
		bitBuf <<= 8;
		bitCnt -= 8;
	}
...
static int jo_processDU(/*FILE*/JO_File *fp, int &bitBuf, int &bitCnt, float *CDU, float *fdtbl, int DC, const unsigned short HTDC[256][2], const unsigned short HTAC[256][2]) { // <- STEVE CHANGE
...
bool jo_write_jpg(/*const char *filename*/JO_File * fp, const void *data, int width, int height, int comp, int quality) { // <- STEVE CHANGE
	...
	if(!data || !fp/*filename*/ || !width || !height || comp > 4 || comp < 1 || comp == 2) {// <- STEVE CHANGE
		return false;
	}
// STEVE CHANGE
	/*
...
// /STEVE CHANGE
	...
	// STEVE CHANGE
	/*fwrite*/fp->Write(head0, sizeof(head0));//, 1, fp);
	/*fwrite*/fp->Write(YTable, sizeof(YTable));//, 1, fp);
	/*putc*/fp->PutC(1);//, fp);
	/*fwrite*/fp->Write(UVTable, sizeof(UVTable));//, 1, fp);
	...
	/*fwrite*/fp->Write(head1, sizeof(head1));//, 1, fp);
	/*fwrite*/fp->Write(std_dc_luminance_nrcodes+1, sizeof(std_dc_luminance_nrcodes)-1);//, 1, fp);
	/*fwrite*/fp->Write(std_dc_luminance_values, sizeof(std_dc_luminance_values));//, 1, fp);
	/*putc*/fp->PutC(0x10);//, fp); // HTYACinfo
	/*fwrite*/fp->Write(std_ac_luminance_nrcodes+1, sizeof(std_ac_luminance_nrcodes)-1);//, 1, fp);
	/*fwrite*/fp->Write(std_ac_luminance_values, sizeof(std_ac_luminance_values));//, 1, fp);
	/*putc*/fp->PutC(1);//, fp); // HTUDCinfo
	/*fwrite*/fp->Write(std_dc_chrominance_nrcodes+1, sizeof(std_dc_chrominance_nrcodes)-1);//, 1, fp);
	/*fwrite*/fp->Write(std_dc_chrominance_values, sizeof(std_dc_chrominance_values));//, 1, fp);
	/*putc*/fp->PutC(0x11);//, fp); // HTUACinfo
	/*fwrite*/fp->Write(std_ac_chrominance_nrcodes+1, sizeof(std_ac_chrominance_nrcodes)-1);//, 1, fp);
	/*fwrite*/fp->Write(std_ac_chrominance_values, sizeof(std_ac_chrominance_values));//, 1, fp);
	// /STEVE CHANGE
	static const unsigned char head2[] = { 0xFF,0xDA,0,0xC,3,1,0,2,0x11,3,0x11,0,0x3F,0 };
	// STEVE CHANGE
	/*fwrite*/fp->Write(head2, sizeof(head2));//, 1, fp);
	// /STEVE CHANGE
	...
	// EOI
// STEVE CHANGE
	/*putc*/fp->PutC(0xFF);//, fp);
	/*putc*/fp->PutC(0xD9);//, fp);
